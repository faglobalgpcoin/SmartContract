...
contract ERC721 is ERC165, IERC721 {
    ...
    */
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        ...
        _clearApprovalAndTransfer(from, to, tokenId);
    }
    ...
    */
    function _clearApprovalAndTransfer(address _from, address _to, uint256 _tokenId) internal {
        require(_to != address(0), "ERC 721 : to address is required ");
        require(_to != ownerOf(_tokenId), "ERC 721 : cannot transfer to same address");
        require(ownerOf(_tokenId) == _from, "ERC 721 : only owner of token can transfer");
    ...
    */
    function _clearApproval(address _owner, uint256 _tokenId) private {
        require(ownerOf(_tokenId) == _owner, "ERC 721");
    ...
    */
    function _removeToken(address _from, uint256 _tokenId) private {
        require(ownerOf(_tokenId) == _from, "ERC 721 : only the onwer of token can remove token");
        ...
        _ownedTokens[_from][lastTokenIndex] = 0;
 ...
 */
contract IDetailedERC721 is IERC721 {
...
//  * @dev see https://github.com/ethereum/eips/issues/721
//  */
contract Adamant is IDetailedERC721, ERC721, Ownable {
  using strings for *;

  // Token name
  string private _name = "ADAMENT";
  ...
  function mint(string memory _newMetadata) public onlyOwner {
    super._mint(msg.sender, _nextTokenId);
    _metadata[_nextTokenId] = _newMetadata;
    emit MetadataUpdated(msg.sender, _nextTokenId, _newMetadata);
    _nextTokenId++;
  }
  ...
  */
  ...
  function checkIsFull(string memory metadata) public pure returns (bool) {
    strings.slice memory s = metadata.toSlice();
  ...
  */
  function addTokenMetadata(uint256 _tokenId, string memory _newMetadata) public onlyOwnerOf(_tokenId) {
    ...
    require(!checkIsFull(oldMetadata), "Cannot add metadata of faceted token");

    _metadata[_tokenId] = withNewData;
    emit MetadataUpdated(msg.sender, _tokenId, withNewData);
  }
}